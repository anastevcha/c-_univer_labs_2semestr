...
std::expected<double, std::string> calculateZ(const double x) {
    ...
    double pi7{ std::numbers::pi / 7.0 };

    double denomInside{ std::abs(expSin - pi7) };

    if (denomInside == 0.0)
        return std::unexpected("Знаменатель равен нулю (e^(sin x) = pi/7)");
...
void task4() {
    ...
    std::cout << "\nЗАДАЧА 4: прямоугольники с площадью K\n";
...
void task5() {
    ...
    std::cout << "Спортсмен пробежит больше 20 км на "
        << day << " день\n";
...
int main() {
    ...
    while (true) {
        ...
        std::cout << "4 - Прямоугольники\n";
        std::cout << "5 - Спортсмен\n";
        std::cout << "6 - Палиндром\n";
